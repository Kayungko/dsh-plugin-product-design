...
Do not use this skill for broad UX critique, design critique, product audits, or flow reviews. Use [$pd-audit](../audit/SKILL.md) for those user-facing requests.

Use this skill before every Product Design build handoff.

A passing QA run requires both:

- a source visual target: image, screenshot, mockup, design frame, written ideation direction, or source capture
...
1. Identify the comparison target.
   - Determine the source design: image, design frame, screenshot, spec, mockup, or written direction.
...
2. Capture evidence.
   ...
   - Capture screenshots at the intended viewport.
...
3. Normalize before comparing.
   ...
   - Normalize image density before comparing. If the source or implementation is double-density, compare against a same-size normalized copy. Record the source pixels, implementation pixels, CSS viewport, and density normalization used in `design-qa.md`.
...
5. Review systematically.
   ...
   - If the source does not address some issue you're seeing (e.g. a null state), call that out as a separate finding as a shortcoming of the source to be addressed.
   - Decide whether the implementation "looks as good" as the source. If there are stylistic problems, call them out. If the user's prompt is leaking into the implementation (vs letting the app stand on its own), call that out as well.
...
- Fonts and typography: family, fallback, weight, size, line height, letter spacing, antialiasing, hierarchy, wrapping, truncation, and whether display text and small UI text use appropriate optical weights.
...
If browser-rendered evidence is required but missing, `final result` is `blocked`.
