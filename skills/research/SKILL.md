...
Use saved product URLs, design files, screenshots, reference images, codebase paths, Storybook links, tokens, design systems, brand assets, component refs, browser preferences, and share targets as grounding material when relevant.

Do not inspect every saved reference. Inspect only the saved references needed for the current task.

## Contract

- Restate the product, audience, time horizon, and research scope before scanning.
- Use public sources by default. Use internal sources only when the user provides access and the request allows it.
...
2. Search public sources with the session's web tools (`web_search`, then `web_fetch` for promising pages):
...
- forums, blogs, reviews, and developer communities where relevant

3. Search internal sources only when the user provides them in the conversation or saved context.
