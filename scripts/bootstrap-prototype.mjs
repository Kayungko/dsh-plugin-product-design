#!/usr/bin/env node
/**
 * Bootstrap a new local prototype from the bundled starter template.
 *
 * Usage:
 *   node bootstrap-prototype.mjs --dest /absolute/path/to/new-prototype
 *
 * The destination must be missing or empty. The template is copied
 * recursively (without node_modules/dist caches), the package name is set
 * from the destination folder name, and an npm quiet-config file is added.
 * Prints a JSON result: { status: "created", root, template }.
 */
...
const availableTemplates = new Set(['prototype']);
...
function setNpmDefaults(root) {
  writeText(path.join(root, '.npmrc'), ['fund=false', 'audit=false', ''].join('\n'));
}
